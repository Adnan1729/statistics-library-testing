...
class TestDistributionWorkflow:
    ...
    def test_empirical_rule_68_95_99(self):
        ...
        prob_1sigma = normal_cdf(mu + sigma, mu, sigma) - normal_cdf(
            mu - sigma, mu, sigma
        )
        prob_2sigma = normal_cdf(mu + 2 * sigma, mu, sigma) - normal_cdf(
            mu - 2 * sigma, mu, sigma
        )
        prob_3sigma = normal_cdf(mu + 3 * sigma, mu, sigma) - normal_cdf(
            mu - 3 * sigma, mu, sigma
        )
...
class TestCombinedWorkflow:
    ...
    def test_data_transformation_pipeline(self):
        ...
        expected_transformed_median = (
            original_median - original_mean
        ) / original_std * new_std + new_mean
